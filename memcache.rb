...
class MemCache
  ...
  def get(key, raw = false)
    with_server(key) do |server, cache_key|
      ...
      (flags,value) = cache_get server, cache_key
  ...
  def set(key, value, expiry = 0, raw = false, flags = 0)
    raise MemCacheError, "Update of readonly cache" if @readonly
    ...
    with_server(key) do |server, cache_key|
      ...
      command = "set #{cache_key} #{flags} #{expiry} #{value.to_s.size}#{noreply}\r\n#{value}\r\n"
        ...
        while line = socket.gets do
          ...
          if line =~ /\ASTAT ([\S]+) ([\w\.\:]+)/ then
            ...
            stats[name] = case name
                          when (value.to_i == 0 and value != 0) then
                            value
  ...
  def cache_get(server, cache_key)
    with_socket_management(server) do |socket|
      ...
      unless keyline =~ /(\d+) (\d+)\r/ then
        server.close
        raise MemCacheError, "unexpected response #{keyline.inspect}"
      end
      flags = $1.to_i
      value = socket.read $2.to_i
      socket.read 2 # "\r\n"
      socket.gets   # "END\r\n"
      return [flags,value]
    end
